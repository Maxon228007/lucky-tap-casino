...
router.post('/claim', (req, res) => {
    ...
    (0, db_1.runQuery)('UPDATE users SET balance = balance + ?, spins_remaining = spins_remaining - 1, total_earned = total_earned + ? WHERE id = ?', [reward, reward, user.id]);
    (0, db_1.runQuery)('INSERT INTO spin_history (user_id, reward) VALUES (?, ?)', [user.id, reward]);
...
router.get('/history', (req, res) => {
    ...
    const history = (0, db_1.queryAll)('SELECT * FROM spin_history WHERE user_id = ? ORDER BY created_at DESC LIMIT 50', [user.id]);
    res.json({ history });
});
