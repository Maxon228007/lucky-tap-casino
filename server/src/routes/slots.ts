...
function getWeightedSymbol(): typeof SLOT_SYMBOLS[0] {
...
function calculateWin(reels: typeof SLOT_SYMBOLS, bet: number): number {
...
router.post('/spin', (req: AuthRequest, res: Response): void => {
  ...
  runQuery(
    ...
    [netChange, bet, user.id]
  );

  const resultSymbols = reels.map(r => r.name).join('|');

  runQuery(
    'INSERT INTO slots_history (user_id, bet, result, win_amount) VALUES (?, ?, ?, ?)',
    [user.id, bet, resultSymbols, winAmount]
  );
...
router.get('/history', (req: AuthRequest, res: Response): void => {
  ...
  const history = queryAll<any>(
    'SELECT * FROM slots_history WHERE user_id = ? ORDER BY created_at DESC LIMIT 20',
    [user.id]
  );
